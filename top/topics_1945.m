...
%%%% Load ETOPO1
   ...
   ncid = netcdf.open('/Users/fengyanshi15/work/TOPICS/bathy/ETOPO1_Ice_c_gmt4.grd','NC_NOWRITE');
...
%%%% Variable declarations and default values
    ...
    xres = 400; yres = 300;
    sVal =  20; sScale = 1; nVal =  26; nScale = 1;
    wVal = 56; wScale = 1; eVal =  67; eScale = 1;

    resVal = 2; resScale = 1/60;
    Mglob = 400; Nglob = 300;

   mypath = '/Users/fengyanshi15/CONSULTING/jacobs/topics_matlab/result/';

    % coseismic parameters
    xoVal = 63.0; xoScale = 1;
    yoVal = 24.5; yoScale = 1;
    strike = 240.0; dip = 5; rake = 90;
    depthVal = 25.0; depthScale = 1e3;
    lengthVal = 126; lengthScale = 1e3;
    widthVal = 38; widthScale = 1e3;
    slip = 7.5;
...
%             drawnow;            
%             
%             saveas(h,fullfile(mypath,'bathy'),myfmt);
%             close(h);
%         end
%         if get(hhf,'Value')
%             
...
%             colormap(ha,my_cmap);
%             colorbar('peer',ha);
%             saveas(h,fullfile(mypath,'h'),myfmt);
%         end
%         if get(huf,'Value')
%             imagesc(my_x,my_y,my_z,'Parent',ha);
%             title(ha,my_title);
...
%             colormap(ha,my_cmap);
%             colorbar('peer',ha);
%             saveas(h,fullfile(mypath,'u'),myfmt);
%         end
%         if get(hvf,'Value')
%             imagesc(my_x,my_y,my_z,'Parent',ha);
%             title(ha,my_title);
...
%             colormap(ha,my_cmap);
%             colorbar('peer',ha);
%             saveas(h,fullfile(mypath,'v'),myfmt);
%         end
%     end
...
% this is yjchens result
yy = deg2km(distance(yo,xo,my_y,xo)).*sign(my_y-yo);
xx = deg2km(distance(yo,xo,yo,my_x)).*sign(my_x-xo);
[E,N] = meshgrid(xx,yy);
